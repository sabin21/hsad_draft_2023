...
// import vertex from '../public/shader/vertex-01.glsl'
// import fragment from '../public/shader/fragment-01.glsl'
...
function init(){
  ...
  plane1.position.set(0.5,0.2,-0.5);
  plane2.position.set(-1.1,-0.8,-1.5);

  clock = new THREE.Clock();

  const geom = new StairsGeom(3);
  const mat = new THREE.MeshLambertMaterial({ 
    alphaMap: image_tex, alphaTest: 0.1, 
    map: image_tex, 
    // side: THREE.DoubleSide
  });
const mesh = new THREE.Mesh(geom, mat);
mesh.position.set(-0.3, 0.2,0);
mesh.rotation.set(Math.PI*0.1, Math.PI*0.25, 0);
scene.add(mesh)
